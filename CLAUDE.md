...
- **`tags` は CSV「カテゴリー」列が4軸（季節 / ジャンル大分類 / 内外 / 同行者）を連結したもの + ジャンル1/2 をフラットに1配列へ詰めた形**（位置情報は無く、複合値は「・」連結のまま）。/select のタグフィルターはこれを実行時に「ジャンル大分類」「同行者」の2軸へ解釈する（`src/lib/spotTags.ts`）。同行者は「・」で分解（`全構成`→全トークン展開）、ジャンルは `カフェ・レストラン`/`温泉・宿`/`歴史・文化` を1単位として守り他は分解（最長一致）。**`tags` が空のスポット（例: 那須温泉神社）はフィルター有効時に隠れる** → CSVにカテゴリを足して再生成すれば直る。
