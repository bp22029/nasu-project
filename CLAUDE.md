...
- ✅ **グリッド連携**: 掲載許可された投稿写真を /select のカードに Google 写真とあわせて表示（初期表示ランダム + 自動切替つきカルーセル。セクション14参照）
...
### 動かす条件（すべて満たすときだけタイマーを回す）

| 条件 | 理由 |
|---|---|
| 表示できる写真が2枚以上 | 1枚しかないカードでタイマーを持たない（約200枚のグリッドで無駄な `setInterval` を作らない） |
| カードが画面内（`inView`） | 画面外で回し続けても意味がない。IntersectionObserver は **disconnect せず**監視を継続する（写真取得のトリガー `visible` は一度 true にしたら戻さないので、再取得は起きない） |
| hover していない | **カードは選択コントロールでもある**。触っている最中に絵が変わると、気に入った写真でタップしたつもりが別の写真になる |
| 未選択 | 選んだ後に絵が変わらないようにする（同上） |
| `prefers-reduced-motion: reduce` でない | 動きを減らす設定の尊重。購読するのは写真2枚以上のカードだけ（全カードにリスナーを張らない） |

開始タイミングは `idHash(spot.id) % AUTO_ADVANCE_PHASE_MS` でカードごとにずらす。
写真ありカードが同フレームで一斉に切り替わるとデコードが集中し、見た目も機械的になるため。

### 描画と通信量の抑え方

- **DOM に置く写真は常に最大2枚**（表示中 + 直前の1枚）。全枚数を重ねて先読みすると、投稿写真は長辺1600pxのJPEGなので通信量が数MB増える。
- **先読みは次の1枚だけ**。読み込めたURLを `preloadedRef` に記録し、**次の1枚が読めていないときは切替を見送る**。
  読めていないまま進めると、透明な新レイヤーが前の写真を隠してカードが下地に抜ける（実装中に実際に起きた）。
- **フェード開始は新しい画像の `onLoad`**。時間で始めると同じ「下地に抜ける」事故になる。取り逃し用に 2.5 秒のフォールバックだけ置く。
- **下地のグラデーションは常に敷く**。写真の読み込み中も写真なしのときもここが見えるので、カードが生成りの背景色に白く抜けることがない。
- 使うのは opacity の transition だけで GPU 合成に収まる。セクション11で禁止した `blur` / `backdrop-filter` / `mix-blend-mode` / rAF ループとは別物。

### 手動操作との関係

右端の上下ボタン（`stepPhoto`）は従来どおり。デスクトップではボタンに触れる時点で hover 状態になり自動切替は止まる。
シャッフルボタンによる写真の選び直し（`shuffleNonce`）も従来どおりで、自動切替とは独立して動く。
