...
- **旅タイプ診断**: 16問の5件法に答えると、4つの視点（計画↔即興 / 刺激↔癒し / 内向↔外向 / 体験↔形）から旅タイプ（動物マスコット）を診断（`/diagnosis`、機能2）。各軸の傾きも%で表示
- **診断結果でスポットを並べ替え**: 診断結果画面の「このタイプで旅を設計する」から `/select` へ遷移すると、タイプの傾き（刺激↔癒し・体験↔形の2軸）に合ったジャンルのスポットが「おすすめ順」で上位に並ぶ。傾きが強い軸ほど並びに強く効く。ジャンルフィルターは隠れ「だれと」フィルターは併用可。バッジの「おすすめ順を解除」で通常表示に戻る（スポットを選ぶのはユーザー。ルート自動生成はしない）
- **ルート設計**: 選択したスポットをTSP最適化で並び替え、ORS（OpenRouteService）で道なり経路を計算
- **巡回順の一部固定**: ルート結果のタイムラインで各スポットの訪問順を「自動 / N番目」から選ぶと、そのスポットを指定の順番に固定して残りを自動で最短最適化（例: ご飯屋を1番目に）。固定情報はルートURL（`lock=`）に載るので共有・リロードでも維持される
- **出発地の選択**: 那須塩原駅 / 道の駅 那須高原友愛の森 / 那須IC / GPS現在地
- **周遊 / 片道**: 出発地へ戻るルートと、開いたルートの切り替え
- **有料道路の回避**: トグルで一般道のみに限定可能
- **共有できるルートURL**: ルート結果は `/route?spots=..&dep=..&trip=..&tolls=..&lock=..` のURLで表現され、リンク共有・リロード復元ができる（`lock` は巡回順の固定。任意）
- **ルートをSNSで共有**: ルート結果画面の「ルートを共有する」ボタンから、表示中のルートをリンクとして共有できる。スマホではOSの共有シート（Web Share API）でLINE / Instagram / X / メール等へ、PCではリンクのコピー・LINE・X が選べる（共有するのはアプリ内のルートURLのみ）
- **グローバルナビ**: どのページからも右上の「N A S U」で1タップでホームへ。MENU から全ページ（ルート設計・旅タイプ診断・みんなの旅・写真投稿・旅の記録・マイページ）へ直接遷移できる
