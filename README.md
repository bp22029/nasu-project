...
- **診断結果の保存・共有**: 結果画面の「結果を保存」で診断結果を残せる（非公開・自分用。ニックネーム不要）。マイページ（`/me`）の「DIAGNOSIS」から**最新の結果1件**を見直す・もう一度共有・この結果で旅を設計・削除できる。「結果を共有する」で結果URLを共有でき、開いた人には結果カードと「自分も診断する」が表示される
...
- **ルートを保存して見返す**: ルート結果画面の「ルートを保存」ボタンで、設計したルートを軽量ブックマークとして残せる（写真なし・非公開・自分用。ニックネーム不要）。保存したルートはマイページ（`/me`）の「SAVED ROUTES」から一覧でき、タップで地図つきルート画面を復元。名前はスポット名から自動でつき、あとで改名もできる
...
- **マイページ**: 保存したルートの見返し・改名・削除、自分の投稿の確認・編集（文章と掲載許可）・削除、ニックネームの変更ができる（`/me`）。写真を削除するとスポット選択のグリッドからも消える
